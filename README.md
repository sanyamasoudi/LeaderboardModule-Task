...
### Trade-off‌ها
- **افزایش Boilerplate Code**: در مقایسه با MVC، به دلیل استفاده از Interfaceها، تعداد کلاس‌ها و حجم کد اولیه بیشتر است.
- **مدیریت دستی چرخه حیات**: در MVP توسعه‌دهنده باید در لایه View (مثلاً `onDestroy`) اتصال Presenter را به صورت دستی قطع کند تا از نشت حافظه (Memory Leak) جلوگیری شود.
- **اتصال یک‌به‌یک (1:1 Mapping)**: برخلاف MVVM، در MVP معمولاً هر Presenter به یک View خاص وابسته است.
...
1. **صحنه (Scene) اصلی پروژه** (`MainScene.unity`) را باز کنید.
...
1 - **استفاده از ScriptableObjects برای تنظیمات**  
   برای مدیریت `ApiConfig` از `ScriptableObject` استفاده شد تا تغییر پارامترهایی مانند `BaseURL`، `Timeout` و `MaxRetryCount` بدون نیاز به تغییر کد و در حین اجرا (برای پلتفرم‌های مختلف) امکان‌پذیر باشد.
